...
do
    ...
    if ! ./build.sh; then
        ...
        git_no_tty add .
        git_no_tty commit -m "Temporary commit that should not get pushed to github"
        ...
        if ./build.sh; then
            ...
            if ./build.sh -d; then
                ...
                git_no_tty reset 'HEAD~1'
                git_no_tty add .
                git_no_tty commit -m "Regenerated library to pick up latest schema changes"
                if git_no_tty push "git@github.com:taskcluster/${repo}.git" master; then
                    ...
                    if [ "${repo}" == 'taskcluster-client-java' ]; then
                        ...
                        mvn javadoc:javadoc
                        git_no_tty checkout gh-pages
                        rm -rf apidocs
                        mv target/site/apidocs .
                        git_no_tty add apidocs
                        git_no_tty commit -m "Regenerated javadocs after api update(s)"
                        if git_no_tty push "git@github.com:taskcluster/${repo}.git" gh-pages:gh-pages; then
                            say -v Samantha "New javadocs published"
                        else
                            say -v Samantha "Could not publish updated javadocs"
                        fi
                        git_no_tty reset master
                        git_no_tty checkout -f master
                        git_no_tty clean -fd
                        ...
                        if git_no_tty tag -m "Released version ${version}" "v${version}"; then
                            say -v Samantha "Tagged locally"
                            if git_no_tty push "git@github.com:taskcluster/${repo}.git" "refs/tags/v${version}:refs/tags/v${version}"; then
